...
use App\Model\{Admin, Article, Menu};
...
class IndexController extends Controller
{
    ...
    public function login(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            // $img = Captcha::src('flat');
            return view('admin.login', ['img' => $img]);
        }
        ...
        if (!empty($name) && !empty($pass)) {
            if ($find = Admin::select('id', 'name', 'password as pass')->where('name', $name)->firstOrFail()) {
                $find = $find->toArray();
                if (Hash::check($pass, $find['pass'])) {
                    ...
                    // 将登录人的菜单权限和路由权限查出并存储在session
                    $data['menus'] = Menu::treeForMenuForAdmin($find['id'])['menus'];
                    $data['permissions'] = Menu::treeForMenuForAdmin($find['id'])['permissions'];

                    //

                    session()->put('_admin', $data);
                    $returnData = config('json.simple');
                } else {
                    $returnData['text'] = '用户名或密码错误';
                }
            } else {
                $returnData['text'] = '用户名或密码错误';
            }
    ...
    {
        // echo '<pre>';
        // $menu = file_get_contents(base_path('public') . '/static/admin/menu.json');

        // $menu = Menu::treeForMenu();
        $menu = session()->get('_admin')['menus'];
        $menu = [
            'status' => 0,
            'msg' => 'OK',
            'data' => $menu,
    ...
    public function permission(Request $request)
    {
        ...
        // 非超管需要验证权限
        if ($admin['id'] != 1 && !in_array($url, $admin['permissions'])) {
            ...
            return response()->json(config('json.simple'));
        }
    }
}
